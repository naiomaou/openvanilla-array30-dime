...
一份以 OpenVanilla 行列輸入法為基礎的修改，包含四項 DIME 化調整：

1. **空白鍵送出自訂詞組**：先查自建詞庫、再查主碼表，自訂詞組永遠排在候選列最前面。
2. **自訂詞組優先、且不干擾候選字編號的選字邏輯**：自訂詞組對應 `'` 選字鍵、不占用數字鍵，原碼表候選維持官方位置。例，自訂詞組以 `xx` 組字時：`xx` ＋空白＋空白＝ `自訂詞組` ；`xx` ＋ `1` ＝ `絲` ，數字鍵仍對應原候選字清單。
3. **可隨時編修的自建詞庫**：便利的熱重載機制。編輯詞庫後存檔，切換視窗或輸入法立即生效，不必重新編譯輸入法或新啟動系統。
4. **自訂詞組突破四碼上限**：自訂詞組字碼最長可到 32 鍵。
...
| `LegacyOVIMArray.cpp` | 修改後的完整原始碼（patch 無法套用時的備案） |
...
| 輸入 | 預期結果 |
|---|---|
| `xx` ＋空白＋空白 | 測試詞（自訂詞組，空白鍵直送） |
| `xx` ＋數字鍵 | 原候選字（數字鍵位置不受自訂詞組影響） |
| `oh` ＋ `7` | 留（官方碼表第 7 位，前面有 ⎔ 佔位符） |
| 沒有自訂詞組的任意字碼 | 候選順序與官方行列完全一致 |
...
- 字碼上限 32 鍵。
- 同一組字盡量只對應一筆自訂詞組（多掛時僅第一條能用空白鍵送出，其餘要靠 `字碼＋'＋數字`）。
- 檔案必須是 UTF-8 純文字。
